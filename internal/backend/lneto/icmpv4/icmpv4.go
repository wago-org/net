...
func (a *Adapter) TryEcho(request icmpns.Request) (nscore.Resource, nscore.Progress, error) {
	...
	if !request.Valid() || len(request.Payload) > a.config.MaxPayloadBytes {
...
func (a *Adapter) ingressLocked(frame []byte) (bool, error) {
	...
	if version != 4 || headerWords < 5 || ipFrame.Protocol() != lneto.IPProtoICMP || netip.AddrFrom4(*ipFrame.DestinationAddr()) != a.ipv4Address {
		return false, nil
	}
