...
func TestDescriptorInstallsCompleteBindingsAndPreservesBackend(t *testing.T) {
	...
	backend := plugin.NewBackend(plugin.BackendLnetoV1,
		...
		func(target any) (nscore.Service, error) {
			...
			return nscore.Service{Key: "test-dns", Value: serviceValue}, nil
		},
	)
	...
	if err != nil || !installed || service.Key != "test-dns" || service.Value != serviceValue {
	...
	assertDNSBindings(t, Bindings(plugin.Host{}), 6)
}
...
func assertDNSBindings(t testing.TB, bindings []plugin.Binding, want int) {
	t.Helper()
