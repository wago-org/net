...
func TestDescriptorInstallsCompleteBindingsAndPreservesBackend(t *testing.T) {
	...
	backend := plugin.NewBackend(plugin.BackendLnetoV1,
		...
		func(target any) (nscore.Service, error) {
			...
			return nscore.Service{Key: "test-tcp", Value: serviceValue}, nil
		},
	)
	...
	if err != nil || !installed || service.Key != "test-tcp" || service.Value != serviceValue {
	...
	assertTCPBindings(t, Bindings(plugin.Host{}), 11)
}

func assertTCPBindings(t testing.TB, bindings []plugin.Binding, want int) {
	t.Helper()
