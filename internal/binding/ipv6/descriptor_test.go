...
func TestDescriptorInstallsCompleteBindingsAndPreservesBackend(t *testing.T) {
	...
	backend := plugin.NewBackend(plugin.BackendLnetoV1,
		...
		func(target any) (nscore.Service, error) {
			...
			return nscore.Service{Key: "test-ipv6", Value: serviceValue}, nil
		},
	)
	...
	if err != nil || !installed || service.Key != "test-ipv6" || service.Value != serviceValue {
	...
	assertIPv6Bindings(t, Bindings(plugin.Host{}), 3)
}

func assertIPv6Bindings(t testing.TB, bindings []plugin.Binding, want int) {
	t.Helper()
