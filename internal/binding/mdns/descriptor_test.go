...
func TestDescriptorInstallsCompleteBindingsAndPreservesBackend(t *testing.T) {
	...
	backend := plugin.NewBackend(plugin.BackendLnetoV1,
		...
		func(target any) (nscore.Service, error) {
			...
			return nscore.Service{Key: "test-mdns", Value: serviceValue}, nil
		},
	)
	...
	if err != nil || !installed || service.Key != "test-mdns" || service.Value != serviceValue {
	...
	assertMDNSBindings(t, Bindings(plugin.Host{}), 10)
}

func assertMDNSBindings(t testing.TB, bindings []plugin.Binding, want int) {
	t.Helper()
