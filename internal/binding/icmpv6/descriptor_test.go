...
func TestDescriptorInstallsCompleteBindingsAndPreservesBackend(t *testing.T) {
	...
	backend := plugin.NewBackend(plugin.BackendLnetoV1,
		...
		func(target any) (nscore.Service, error) {
			...
			return nscore.Service{Key: "test-icmpv6", Value: serviceValue}, nil
		},
	)
	...
	if err != nil || !installed || service.Key != "test-icmpv6" || service.Value != serviceValue {
		t.Fatalf("InstallBackend = %+v %v %v", service, installed, err)
	}
	assertICMPv6Bindings(t, Bindings(plugin.Host{}), 14)
}

func assertICMPv6Bindings(t testing.TB, bindings []plugin.Binding, want int) {
	t.Helper()
