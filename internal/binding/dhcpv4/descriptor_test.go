...
func TestDescriptorInstallsCompleteBindingsAndPreservesBackend(t *testing.T) {
	...
	backend := plugin.NewBackend(plugin.BackendLnetoV1,
		...
		func(target any) (nscore.Service, error) {
			...
			return nscore.Service{Key: "test-dhcpv4", Value: serviceValue}, nil
		},
	)
	...
	if err != nil || !installed || service.Key != "test-dhcpv4" || service.Value != serviceValue {
		t.Fatalf("InstallBackend = %+v %v %v", service, installed, err)
	}
	assertDHCPv4Bindings(t, Bindings(plugin.Host{}), 7)
}

func assertDHCPv4Bindings(t testing.TB, bindings []plugin.Binding, want int) {
	t.Helper()
