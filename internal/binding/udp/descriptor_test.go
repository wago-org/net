...
func TestDescriptorInstallsCompleteBindingsAndPreservesBackend(t *testing.T) {
	...
	backend := plugin.NewBackend(plugin.BackendLnetoV1,
		...
		func(target any) (nscore.Service, error) {
			...
			return nscore.Service{Key: "test-udp", Value: serviceValue}, nil
		},
	)
	...
	if err != nil || !installed || service.Key != "test-udp" || service.Value != serviceValue {
	...
	assertUDPBindings(t, Bindings(plugin.Host{}), 6)
}

func assertUDPBindings(t testing.TB, bindings []plugin.Binding, want int) {
	t.Helper()
