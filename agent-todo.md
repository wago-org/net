...
- Reviewed Wago main base: `2fbb34a50e89faad0f2ea4d47a219218d0cd2871` (tree `42ddd8148a73d0a0bd2faccb03c834cfa06e2df3`, parent `18615546584ec09e607856a0da99851656f5be80`, 2026-07-11). Its sole delta from the prior reviewed base is `cli/wagocli/plugin_build.go`. Current integrated review `da4db3c97c643b5385cbca02ec125822afd82abd` (tree `5a538aee28e7a8ff85003dfc35f0f8fc6147fed3`) preserves lineage through patch-equivalent preview-1 fix port `2a9bf214957f47137287f7138cbdcaa634f2715b`, lifecycle review `cf2409d32d7bac858a30422b329984e65cca8fd7`, and that exact main base. `da4db3c9` preserves managed worker table callbacks by directly invoking local wrapper descriptors. The prior integrated chain `540c453d -> 90018dad -> 8131d967 -> 18615546` and earlier pair `7fbc00a57624b26ba8d528d97b419b670e85f64b` / `e44b1baa6eabfba07967a4458fdb56983cb054ae` remain historical evidence only.
...
A current-main Wago review worktree on `net/current-plugin-lifecycle-2fbb34a5` at
`da4db3c97c643b5385cbca02ec125822afd82abd` preserves lifecycle replay
`cf2409d3` on fetched main `2fbb34a50e89faad0f2ea4d47a219218d0cd2871`,
patch-equivalent preview-1 fix port `2a9bf214`, and managed-wrapper integration
`da4db3c9`. `HostImportAccess.CallerResolver`
...
The exact-object compatibility review is green: integrated Wago `da4db3c9` and
...
current fix port `2a9bf214` have the same stable patch ID; current integration
`da4db3c9` passes current WASI `cbdb9b32` without regressing managed callbacks.
The moving-ref topology gate now binds CLI-only upstream child `2fbb34a5` and
continues to fail closed on any later movement.
...
2. Publish current integrated Wago review `da4db3c9` and networking review at exact
...
4. Rerun the strict complete release gate on each final candidate subject using
   ...
   preservation fixture and the integrated Wago `da4db3c9`, selective networking
...
- The pinned production Wago line's `src/wago` tests still need a temporary
  test-only `trapCode` helper; current Wago main `2fbb34a5` does not have that
  historical defect. The helper is removed by the release gate.
- Integrated review `da4db3c9` preserves the exact chain through fix port
  `2a9bf214`, hardened lifecycle review `cf2409d3`, and reviewed Wago main
  `2fbb34a50e89faad0f2ea4d47a219218d0cd2871`; Wago owns exact callback slot
...
- Current integrated Wago `da4db3c9` passes the complete standard-Go suite,
  ...
  `cf2409d3` on main `2fbb34a5` and patch-equivalent fix port `2a9bf214`.
- Current networking review `173b38a` passes standard Go, race, vet, and TinyGo
  against exact Wago `da4db3c9`. Inspection grants ordinary networking only
...
1. Re-fetch publication refs and check whether exact production fix `5c7f76db`,
   refreshed current Wago `da4db3c9`, and networking `173b38a` have immutable
